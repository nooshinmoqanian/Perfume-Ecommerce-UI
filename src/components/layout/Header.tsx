      ...
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-3">
        <div className="flex items-center gap-3">
          <Link to="/" className="rounded-md px-2 py-1 text-base font-semibold text-violet-900 hover:bg-violet-50">
            <div className="flex items-center gap-3">
              <div className="h-9 w-36 flex items-center justify-center rounded bg-gray-100">
                <img src="/logo.png" alt="لوگو" className="max-h-6 object-contain" onError={(e) => {(e.currentTarget as HTMLImageElement).style.display = 'none';}} />
                <span className="text-sm">لوگو شما</span>
              </div>
            </div>
          </Link>
        ...
        <div className="flex items-center gap-2">
          {token ? (
            <>
              <Link to="/account" className="rounded-md px-3 py-2 text-sm text-violet-900 hover:bg-violet-50">
                حساب من
              </Link>
              <button
